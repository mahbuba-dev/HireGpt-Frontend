...
export const useRoomMessages = (roomId?: string) => {
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: async (message, _payload, context) => {
      if (!roomId) {
        return;
      }
      ...
      // Notification logic: notify the other participant with the message text
      try {
        // Find the other participant in the room
        const room = queryClient.getQueryData<any>(["chat-rooms"]);
        ...
        if (otherUserId) {
          await createNotification({
            ...
            userId: otherUserId,
            role: otherRole,
            message: `New message: ${resolvedMessage.text ?? "(attachment)"}`,
          });
        }
      } catch (err) {
