...
export default function TestimonialsDirectory({ testimonials }: TestimonialsDirectoryProps) {
  ...
  return (
    <section className="premium-card section-spacing section-padding">
      <div className="mb-6 section-grid lg:flex-row lg:items-end lg:justify-between">
        <div className="text-container space-y-2">
          <Badge className="border-white/20 bg-white/10 text-white hover:bg-white/10">
            Testimonials archive
          </Badge>
          <h1 className="h1 hero-gradient">What clients say after booking experts on HireGPT</h1>
          <p className="text-base muted">Search, filter, and sort testimonials by expert, rating, and date.</p>
        </div>
        <div className="rounded-2xl border border-white/20 bg-white/10 px-4 py-2 text-base font-medium text-cyan-100">
          {filteredTestimonials.length} results · {averageRating.toFixed(1)} avg rating
        </div>
      </div>

      <div className="mb-6 section-grid rounded-2xl border border-white/20 bg-white/10 p-4 md:grid-cols-4">
      ...
      <div className="section-grid md:grid-cols-2 xl:grid-cols-4">
        {filteredTestimonials.map((testimonial) => {
          ...
          return (
            <article
              key={testimonial.id}
              className="glass-card group relative overflow-hidden transition-transform duration-300 hover:-translate-y-1"
            >
      ...
      {filteredTestimonials.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-white/30 bg-white/5 p-6 text-center text-base muted">
